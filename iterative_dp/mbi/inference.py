import numpy as np
from iterative_dp.mbi import Domain, GraphicalModel, callbacks
from iterative_dp.mbi.graphical_model import CliqueVector
...
class FactoredInference:
    def __init__(self, domain, backend = 'numpy', structural_zeros = None, metric='L2', log=False, iters=100, warm_start=False, elim_order=None):
        ...
        if backend == 'torch':
            from iterative_dp.mbi.torch_factor import Factor
            self.Factor = Factor
        else:
            from iterative_dp.mbi import Factor
            self.Factor= Factor
